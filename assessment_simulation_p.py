...
omega_dir = 'data/eigenfrequencies'
...
class DataSampler:
    ...
    def __init__(self, d, k, density_matrices, n_samples, omegas=False, ts=False):
    ...
    def sample(self):
        ...
        np.savetxt(f'data/{self.d}_{self.k}.csv', np.real(self.Ps))
...
def test_state(d, k):
    ...
    '''
    Density matrix containing an equal superposition of k basis states.
    '''

    # normalised state vector
    v = np.concatenate((np.ones(k) / np.sqrt(k), np.zeros(d-k)))
    np.random.shuffle(v)

    v = np.array(v, dtype=complex)

    # density matrix
    rho = pure_density_matrix(v)

    return rho


def mixed_in(density_matrices, d, k):

    density_matrices[k] = []
    t_state = test_state(d, k)
    ps = np.array([0, 0.5, 0.9, 0.95, 0.99, 0.995, 0.996, 0.997, 0.998, 0.999, 1])

    for p in ps:
        d_mat = ((1 - p) * t_state) + ( (p / d) * np.identity(d))
        density_matrices[k].append(d_mat)
...
def get_density_matrices(d, ks):
    ...
    for k in ks:
        mixed_in(density_matrices, d, k)

    return dict(sorted(density_matrices.items()))



def execute2(samplers, d, k, density_matrices, n_samples, omegas):
    ds = DataSampler(d, k, density_matrices[k], n_samples, omegas, ts)
    ds.sample()
    # samplers.append(ds)


def ramsey(d, ks, density_matrices, n_samples, omegas):
    ...
    for k in ks:
        ...
        p = Process(target=execute2, args=[samplers, d, k, density_matrices, n_samples, omegas])
        p.start()
        processes.append(p)
...
if __name__ == '__main__':
    ...
    density_matrices = get_density_matrices(d, ks)
    ramsey(d, ks, density_matrices, n_samples, omegas)
